# -*- coding: utf-8 -*-
"""
Created on Mon Sep  5 20:21:52 2022
...
#Importing Dataset
df = pd.read_csv(r'C:\Users\mavul\Downloads\Circular saw - material costs.csv')
df
...
ax.scatter(df['Cost Matl Unit'].values, df['Cost Total Unit'].values,marker='o', color='r', alpha=1, linewidth=1, 
            edgecolor='k')
ax.set_title('Relationship between Cost Matl Unit  x Cost Total Unit', fontsize=15)
...
#Splitting Data
X=df.drop('Cost Total Unit', axis=1)
y=df['Cost Total Unit']
...
plt.title('Circular saw - material costs (Training dataset)')
plt(0.5, 1.0, 'Circular saw - material costs (Training dataset)')

plt.scatter(X_test, y_test, color = 'gray')
plt.plot(X_test, lr_regressor.predict(X_test), color = 'red')
plt.ylabel('{} ($)'.format(df.columns[1]))
plt.xlabel('{}'.format(df.columns[0]))
plt.title('Circular saw - material costs (Test dataset)')
